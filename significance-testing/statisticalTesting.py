        ...
        if not single_dir.endswith('.json'):
            if embed in single_dir and not 'random' in single_dir:
                ...
                try:
                    ...
                    alt_name = definition[0] + '-gaze' + '_' + definition[1] + '_' + definition[2] + '_' + definition[3]
...
def save_scores(emb_scores, emb_filename, base_scores, base_filename):
    ...
    emb_file = open(config.result_dir + 'tmp/gaze/' + emb_filename, 'w')
    base_file = open(config.result_dir + 'tmp/gaze/' + base_filename, 'w')
...
def main():
    ...
    result_dir = config.result_dir + 'gaze/'
    ...
    hypotheses = [1 for filename in os.listdir(config.result_dir + 'tmp/gaze/') if 'embeddings' in filename]
    ...
    report = '/Users/norahollenstein/Desktop/PhD/projects/testing/reports/gaze/' + config.test + '.json'

    for filename in os.listdir(config.result_dir + 'tmp/gaze/'):

        if 'embeddings' in filename:
            model_file = config.result_dir + 'tmp/gaze/' + filename
            baseline_file = config.result_dir + 'tmp/gaze/' + 'baseline_' + filename.partition('_')[2]
