...
def save_scores(emb_scores, emb_filename, base_scores, base_filename):
    ...
    emb_file = open(config.result_dir + 'tmp/eeg/' + emb_filename, 'w')
    base_file = open(config.result_dir + 'tmp/eeg/' + base_filename, 'w')
...
def main():
    ...
    result_dir = config.result_dir + 'eeg/'
    gaze_datasets = ['n400', 'ucl', 'zuco', 'naturalspeech']
    ...
    hypotheses = [1 for filename in os.listdir(config.result_dir + 'tmp/eeg/') if 'embeddings' in filename]
    ...
    report = '/Users/norahollenstein/Desktop/PhD/projects/testing/reports/eeg/' + config.test + '.json'

    for filename in os.listdir(config.result_dir + 'tmp/eeg/'):

        if 'embeddings' in filename:
            model_file = config.result_dir + 'tmp/eeg/' + filename
            baseline_file = config.result_dir + 'tmp/eeg/' + 'baseline_' + filename.partition('_')[2]
